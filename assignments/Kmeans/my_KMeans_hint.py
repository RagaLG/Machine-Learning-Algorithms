...
class my_KMeans:
    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            ...
            # Initialize the first cluster center by randomly selecting a data point
            cluster_centers = [X[np.random.choice(n_samples)]]

            while len(cluster_centers) < self.n_clusters:
                # Calculate the squared distance of each data point to the nearest cluster center
                distances = [min([np.linalg.norm(x - c) for c in cluster_centers]) for x in X]
                # Calculate the probability distribution for choosing the next cluster center
                probabilities = distances / np.sum(distances)
                # Randomly choose the next cluster center based on the probabilities
    ...
    def fit_once(self, X):
        ...
        for i in range(self.max_iter+1):
            ...
            cluster_centers = [np.mean(cluster, axis=0) if cluster else center for center, cluster in zip(cluster_centers, clusters)]
